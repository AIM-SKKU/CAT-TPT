...
def encode_text_with_learnt_tokens(self, text, asterix_token, learnable_codes, base_token = None, is_emb = False, is_token_emb = False):
    """
    ...
    if base_token is None:
        for i in range(len(text)):
            if is_emb:
                ...
                x_i_longer = torch.cat((x[i][:inds_for_insert[1][i]], learnable_codes[i].weight[i].unsqueeze(0), x[i][inds_for_insert[1][i]+1:]), 0)
            else:
                x_i_longer = torch.cat((x[i][:inds_for_insert[1][i]], learnable_codes[i], x[i][inds_for_insert[1][i]+1:]), 0)
            x[i] = x_i_longer[:num_tokens] #[K,77,C]
    ...
    # x[torch.arange(x.shape[0]), text.argmax(dim=-1)]:[5,512]
    x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection

    return x

# def encode_text_with_learnt_tokens(self, text, is_token_emb = False):
#     """
#         CLIP text encoder replacing the "asterix_token" with learnable_codes
#
#         This function has been modified from a file in the following repository:
