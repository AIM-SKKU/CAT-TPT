...
class PromptLearner(nn.Module):
    ...
    def reset_classnames(self, classnames, class_des_label, n_cls, arch):
        self.n_cls = len(classnames)
        self.n_cls_global = n_cls
        if not self.learned_cls:
            classnames = [name.replace("_", " ") for name in classnames]
            name_lens = [len(_tokenizer.encode(name)) for name in classnames]
        ...
        self.classnames = classnames
        self.class_des_label = class_des_label
...
class ClipTestTimeTuning(nn.Module):
    ...
    def reset_classnames(self, classnames, class_des_label, n_cls, arch):
        self.prompt_learner.reset_classnames(classnames, class_des_label, n_cls, arch)
    ...
    def inference(self, image):
        ...
        ###### Get averaged text features
        text_features = self.get_text_features()
        class_des_label = self.prompt_learner.class_des_label
        text_features = [text_features[class_des_label == i].mean(dim=0, keepdim=True) for i in range(self.prompt_learner.n_cls_global)]
        text_features = torch.cat(text_features, dim=0)

        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ text_features.t()#[64,200]
