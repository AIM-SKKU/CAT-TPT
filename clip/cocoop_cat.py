...
from .custom_clip_cat import TextEncoder
...
class CoCoOpCLIP(nn.Module):
    ...
    def forward_ctx(self, image_features, ctx):
        ...
        class_des_label = self.prompt_generator.class_des_label
        text_features = [text_features[class_des_label == i].mean(dim=0, keepdim=True) for i in range(self.prompt_generator.n_cls_global)]
